...
export default function DataScreen() {
  ...
  const populationRadarData = data.ocean_model.population_average.map(
    (item: OceanDataPoint) => item.value
  );
  const populationRadarLabels = data.ocean_model.population_average.map(
    (item: OceanDataPoint) => item.label
  );
  ...
  const leaderboardBarData: BarChartDataPoint[] = data.leaderboard.map(
  ...
  const investLineData: LineChartDataPoint[] = data.choice_distribution.map(
    (round: ChoiceDistribution) => ({
  ...
  const cashOutLineData: LineChartDataPoint[] = data.choice_distribution.map(
    (round: ChoiceDistribution) => ({
  ...
  const avgInvestmentLineData: LineChartDataPoint[] =
    data.average_investment.map((round: AverageInvestment) => ({
  ...
  return (
    ...
    >
      ...
      <View style={styles.section}>
        ...
        <Text style={styles.chartDescription}>Players ranked by balance</Text>
        <View style={styles.chartContainer}>
          <BarChart
            ...
            noOfSections={4}
            maxValue={Math.max(
              ...data.leaderboard.map((p: LeaderboardPlayer) => p.balance)
            )}
        ...
        {data.trust_vs_investment.length > 0 ? (
          <>
            <Text style={styles.note}>
              📌 Showing correlation between opponent trust scores and
              investment amounts. Data points: {data.trust_vs_investment.length}
            </Text>
            <View style={styles.dataPreview}>
              <Text style={styles.dataTitle}>Sample Data Points:</Text>
              {data.trust_vs_investment
                .slice(0, 5)
                .map((point: TrustInvestmentPoint, index: number) => (
          ...
          <Text style={styles.note}>
            ⚠️ No data available. This requires games where both players
            invested.
          </Text>
        )}
      </View>
